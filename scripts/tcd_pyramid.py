...
def pyramid_noise_like2(x, iterations=5, discount=0.4):
    b, c, h, w = x.shape
    noise = torch.randn_like(x)
    for i in range(iterations):
        r = random.random() * 2.0 + 2.0
        hn = max(1, int(h / (r ** i)))
        wn = max(1, int(w / (r ** i)))
        temp = torch.randn((b, c, hn, wn), device=x.device, dtype=x.dtype)
        up = F.interpolate(temp, size=(h, w), mode="bilinear", align_corners=False)
        noise = noise + up * (discount ** i)
        if hn == 1 or wn == 1:
            break
...
def default_noise_sampler(x, iterations=5, discount=0.4):
    return lambda sigma, sigma_next: pyramid_noise_like2(x, iterations=iterations, discount=discount)


@torch.no_grad()
def sample_tcd_pyramid(model, x, sigmas, extra_args=None, callback=None, disable=None, noise_sampler=None, gamma=0.3, pyramid_iterations=5, pyramid_discount=0.4):
    extra_args = {} if extra_args is None else extra_args
    noise_sampler = default_noise_sampler(x, iterations=pyramid_iterations, discount=pyramid_discount) if noise_sampler is None else noise_sampler
    ...
    if inner is None:
        raise AttributeError("model.inner_model is required for TCD Pyramid (sigma_to_t / t_to_sigma).")
...
if NAME not in [s.name for s in sd_samplers.all_samplers]:
    samplers = [(NAME, sample_tcd_pyramid, [ALIAS], {})]
    data = [
